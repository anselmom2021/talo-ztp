...
- `TALOSCONFIG` points to a talosconfig file (optional). If not set and `TALOS_INSECURE` is false, the server creates `web/data/talosconfig` (you must still configure endpoints).
- `TALOS_INSECURE=true` to add `--insecure` to talosctl commands (needed for maintenance mode and avoids relying on talosconfig endpoints)
